...
class VendorOrder extends Controller
{
    ...
    {
        $order = ModelsVendorOrder::select("*", "vendor_orders.id AS id", "vendor_orders.created_at AS created_at")->join('users', 'users.id', '=', 'vendor_orders.user_id')->orderBy('vendor_orders.id', 'DESC')->get();
        return view('admin.vendor.order.list', ['order' => $order]);
    }

    public function detail($id)
    {
        $order = ModelsVendorOrder::findOrFail($id);
        return view('admin.vendor.order.detail', compact('order'));
    }
    ...
    public function changeStatus($status, $id)
    {
        ...
        if ($status == "1") {  //order proccessing
            ...
            $adminmsg = $order->order_no . " order's comission's, handle by the ";
    ...
    public function orderApprove(Request $request)
    {
        ...
        if ($response) {
            $detail = $order->order_no . " is delivery is changed delivery_by" . $request->delivery_by . " tracking id " . $request->delivery_trackingid;
            $type = 4;
            $link = 'vendor/order/detail/' . $order->id;
            $user = User::find($order->seller_id);
            $vendornotification = new VendorOrderNotification($msg, $type, $link, $detail);
    ...
    public function changeDelivery(Request $request)
    {
        ...
        $response = $order->save();
        if ($response) {
            $msg = $order->order_no . " is delivery is changed";
            $detail = $order->order_no . " is delivery is changed delivery_by" . $request->delivery_by . " tracking id " . $request->delivery_trackingid;
            $type = 4;
            $link = 'vendor/order/detail/' . $order->id;
            $user = User::find($order->seller_id);
