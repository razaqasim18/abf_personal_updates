...
@section('content')
    <!-- Product Style -->
    @if (SettingHelper::getSettingValueBySLug('customized_ticker'))
        <div class="hwrap">
            <div class="hmove">
                <div class="hitem">{{ SettingHelper::getSettingValueBySLug('customized_ticker') }}</div>
            </div>
        </div>
    @endif
    ...
    <script>
        $(document).ready(function() {
            ...
            function fetch_data(page) {
                ...
                let url = "{{ url('') }}" + "/customize/search?page=" + page + "&product=" + product +
                    "&category=" +
                    category +
                    "&price=" + price + "&sort=" + sort + "&other=2";
                $.ajax({
                    url: url,
                    beforeSend: function() {
                        $(".preloader").show();
