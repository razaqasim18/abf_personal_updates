                                    ...
                                    <table class="table table-striped table-hover" id="table-1" style="width:100%;">
                                        <thead>
                                            <tr>
                                                ...
                                                <th>Order Total</th>
                                                <th>Vendor Order Earning</th>
                                                <th>ABF Comission</th>
                                                <th>ABF Comission Amount</th>
                                                <th>Manage By</th>
                                        ...
                                        <tbody>
                                            ...
                                            @foreach ($order as $row)
                                                <tr>
                                                    ...
                                                    <td>
                                                        {{ 'PKR ' . $row->vendor_amount }}
                                                    </td>
                                                    <td>
                                                        {{ $row->commission . ' %' }}
                                                    </td>
                                                    <td>
                                                        {{ 'PKR ' . $row->commission_amount }}
                                                    </td>
                                                    <td>
                                                       @if ($row->is_order_handle_by_admin == 1)
                                                            Admin
                                                        @else
                                                            Vendor
                                                        @endif
                                                    ...
                                                    <td style="display: grid;">
                                                        ...
                                                        <a href="{{ route('admin.vendor.order.detail', $row->id) }}"
                                                            class="btn btn-sm btn-primary">
                                                            <i class="far fa-eye"></i>
                                                        </a>
                                                        <a href="{{ route('admin.vendor.order.print.pdf', $row->id) }}"
                                                            class="btn btn-sm btn-secondary">
                                                            <i class="fas fa-print"></i>
                                                        </a>
                                                        @if ($row->is_order_handle_by_admin)
                                                            <button id="deliveryModal" data-id="{{ $row->id }}"
                                                                class="btn btn-sm btn-warning">
                                                                <i class="fa fa-paper-plane"></i>
                                                            </button>
                                                            <button id="deleteButton" data-id="{{ $row->id }}"
                                                                href="{{ route('admin.vendor.order.delete', $row->id) }}"
                    ...
                    <form id="approvalForm" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group row">
                            ...
                            <input id="delivery_by" type="text"
                                class="col-sm-8 form-control @error('delivery_by') is-invalid @enderror" name="delivery_by"
                                value="{{ old('delivery_by') }}" required>
                        </div>
                        <div class="form-group row">
...
@section('script')
    ...
    <script>
                ...
                swal({
                    ...
                    .then((willDelete) => {
                        if (willDelete) {
                            ...
                            var url = "{{ url('/admin/vendor/order/delete') }}" + '/' + id;
                            $.ajax({
                                url: url,
                                type: 'DELETE',
                                dataType: 'json',
        ...
        function submitApproval() {
            ...
            let url = "{{ url('/admin/vendor/order/change/delivery') }}";
