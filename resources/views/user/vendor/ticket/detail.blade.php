                    ...
                    <div class="card">
                        <div class="card-header">
                            ...
                            <div class="card-header-action">
                                <a href="{{ route('vendor.ticket.reply', $ticket->id) }}" class="btn btn-primary">Add
                                    Reply</a>
                            </div>
                        </div>
                        <div class="card-body">
                                    ...
                                    <li class="media">
                                        @if ($row->user_type == 0)
                                            <img alt="{{ $row->user ? $row->user->name : null }}"
                                                ...
                                                src="{{ $row->user->image ? asset('uploads/user_profile') . '/' . $row->user->image : asset('img/users/user-3.png') }}">
                                        @else
                                            <img alt="{{ $row->admin ? $row->admin->name : null }}"
                                                class="mr-3 rounded-circle" width="70"
                                                src="{{ $row->admin->image ? asset('uploads/admin_profile') . '/' . $row->admin->image : asset('img/users/user-3.png') }}">
                                        @endif
                                        ...
                                        <div class="media-body">
                                            ...
                                            <div class="text-time">{{ date('d M Y h:i A', strtotime($row->created_at)) }}
                                            </div>
